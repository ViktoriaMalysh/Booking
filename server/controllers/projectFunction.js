...
module.exports.addTime = async function(req,res){
  try{
    ...
    const tt = await Project.findAll({where: {projectName: time.projectName}, raw: true}).then(result=>{
      if(result) console.log(result[0].h)
      let tt = result[0].h
      tt=tt+time.h
      const ttt = tt
      const t = ttt.toFixed(0)
        Project.update({ h: t}, {where: {projectName: time.projectName, id_user: time.id}});
...
module.exports.create = async function(req,res){
  try{
    ...
    let project = Project.build({projectName: projects.nameProject, start: 0, end: 0, time: 0, id_user: projects.id}); 
    await project.save(); 
...
module.exports.search = async function(req,res){
  try{
    ...
    await Project.findAll({where: {projectName: projectName}, raw: true}).then(result=>{
